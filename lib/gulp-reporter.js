...
function consoleReporter(errors, file) {
	...
	gutil.log(gutil.colors.bgBlack(gutil.colors.cyan.underline(file.relative.replace(/\\/g, "/")) + consoleMsg.join("")));
}

function mdReporter(file, errors) {
	errors = errors.map(error => {
		let pos;
		if (error.line && error.column) {
			pos = `[${ error.line }:${ error.column }] `;
	...
	errors.unshift("## " + file.relative.replace(/\\/g, "/"));
	return errors.join("\n\n");
}
...
function reporter(file, errors, options) {
	...
	process.nextTick(() => {
		if (options.inConsole || options.inConsole == null) {
			consoleReporter(errors, file);
		}
		if (options.inJSON) {
			options.inJSON(errors, file);
		}
		if (options.inMD) {
			options.inMD(mdReporter(file, errors), file);
		}
		if (options.inHTML) {
			options.inHTML(require("markdown-it")().render(mdReporter(file, errors)), file);
		}
	});
}
	...
	return through.obj({
		...
		if (file.eslint) {
			...
			if (file.eslint.messages.length && !isIgnore(file)) {
				errors = errors.concat(file.eslint.messages.map(msg => {
					return {
		...
		if (file.jshint) {
			...
			if (!file.jshint.success && !file.jshint.ignored && !isIgnore(file)) {
				errors = errors.concat(file.jshint.results.map(result => {
					return {
		...
		if (file.postcss) {
			...
			if (file.postcss.messages.length && !isIgnore(file)) {
				errors = errors.concat(file.postcss.messages.map(msg => {
					return {
