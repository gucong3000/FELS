...
function startgui() {
	if (!guiReadyPromise) {
		...
		guiReadyPromise = startGuiWith("nohup")

		.catch(() => startGuiWith("strat"));
...
/**
 * @param  {String} startcmd 	后台启动程序的命令，默认"nohup"
 * @return {Promise} 进程启动是否成功的Promise对象
 */
function startGuiWith(startcmd) {
	return new Promise((resolve, reject) => {
		const childProcess = require("child_process");
		let cmd = [require("electron-prebuilt"), path.join(__dirname, "gui.js")].map(arg => /\s/.test(arg) ? JSON.stringify(arg) : arg);
		cmd.push("--wait-ready-signal");
		cmd = cmd.join(" ");
		let ls;

		function exec(commd) {
			console.log(commd + " " + cmd);
			ls = childProcess.exec(commd + " " + cmd, {
				timeout: 8000
			}, (error, stdout, stderr) => {
				if (stderr) {
