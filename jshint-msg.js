...
function han(jshintmsg) {
	if (jshintmsg) {
		jshintmsg = require(jshintmsg);
	} else {
		jshintmsg = require("jshint/src/messages.js");
	}
	for (var type in jshintmsg) {
		for (var code in jshintmsg[type]) {
			if (jshintmsg[type][code] && jshintmsg[type][code].desc && msg[jshintmsg[type][code].desc]) {
				jshintmsg[type][code].desc = msg[jshintmsg[type][code].desc] + " → " + jshintmsg[type][code].desc;
			}
		}
	}
}
...
if (process.env.APPDATA) {
	// 将本工具集成到全局安装的jshint中去
	var path = require("path");
	var fs = require("fs");
	var globalJshintDir = path.join(process.env.APPDATA, "npm/node_modules/jshint/src");
	var cliPath = path.join(globalJshintDir, "cli.js");
	...
	fs.readFile(cliPath, function(err, data) {
		if (!err) {
			...
			if (data.indexOf(code) < 0) {
				fs.writeFile(cliPath, data.replace(/((["'])use\s+strict\2;?\n+)/, "$1" + code), function() {
